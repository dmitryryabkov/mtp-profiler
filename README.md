...
- **Apple Silicon aware** - Collects chip type, memory, and thread information
- **Deterministic recommendations** - Algorithmic scoring, no LLM required
...
Long-context efficiency: poor
Stability: unstable
Average generation throughput: 18.38 t/s
Average draft acceptance rate: 97.8%
============================================================
```
...
- **Throughput statistics** - avg, std, min, max, median generation TPS
...
- **MTP setting comparisons** - Grouped by `n_max` setting
...
```bash
mtp-profiler plot <analysis.json> [-d OUTPUT_DIR] [-r RUN_ID] [-v]
```
